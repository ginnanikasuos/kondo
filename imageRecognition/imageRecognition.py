...
class imageRecognition():
    ...
    def search_capture(self):
        print("search_capture")
        os_name = platform.system()
        if os_name == 'Linux':
            cmd = "v4l2-ctl --list-device"
            ret = subprocess.check_output(cmd.split())
            flg = False
            for v in str(ret).split('\\n'):
                if flg:
                    video_num = int(v[-1])
                    break
                if "UVC Camera" in v:
            ...
            print("camera num{0}".format(video_num))
        elif os_name == 'posix':
            for i in range(5):
    ...
    def find_obj(self,cp_img):
        ...
        for i in range(len(contours)):
            ...
            if 1000 < area and hrc[3] == -1 and 200 < length and length < 500:
                ...
                idx,per = self.obj_distinction(cp_img,(cx,cy))
                if idx == False:
                    idx = 0
                    per = 0
                x0 = cx - 10
    ...
    def obj_distinction(self,img,center):
        print("object distinction")
        cx,cy = center
        #print(center)
        x0 = cx - self.pic_size
        y0 = cy - self.pic_size
    ...
    def camera_view(self):
        ...
        while True:
            ...
            # find object
            center,cap_img = self.find_obj(frame)
            # image show
